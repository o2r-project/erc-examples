...
has_rmd <- sapply(X = finished_papers_repro_zips, FUN = function(zipfile) {
  ...
  any(grepl(pattern = "\\.Rmd", x = files_in_zip, ignore.case = TRUE))
})
...
```{r upload}
#upload_workspace_zip(finished_papers_upload[[3]], user_cookie)
...
publish_compendium <- function(id, cookie) {
  cat(format(Sys.time()), "Publishing ", id, "\n")
...
```{r publish}
#id <- upload_workspace_zip(finished_papers_upload[[3]], user_cookie)
#publish_compendium(id, o2r_user_cookie)

finished_papers_publishings <- sapply(X = finished_papers_uploads,
                                  FUN = publish_compendium,
                                  cookie = o2r_user_cookie)
```
...
finished_papers_jobs <- sapply(X = names(finished_papers_publishings),
                                  FUN = finish_job,
                                  cookie = o2r_user_cookie)
