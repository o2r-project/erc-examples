...
author: 
- affiliation: "Geography Department, Geography and Geology Faculty, Alexandru Ioan Cuza University of Iaşi"
  name: "Mihai Niculita"
date: "5 April 2017"
licenses:
  code: CC-BY-4.0
...
``` {r, echo=FALSE, cache=FALSE, results="hide", message=FALSE , warning=FALSE, comment=FALSE}
###################################################################
###################################################################
...
``` {r, echo=FALSE, cache=FALSE, results="hide", message=FALSE , warning=FALSE, comment=FALSE, warning=FALSE}
###################################################################
#------read the shapefile------------------------------------------
###################################################################
...
if (!file.exists("landslide_inventory_mbbox.shp")){
...
if (!file.exists("landslide_inventory_mbbox_points.shp")){
...
midspdf <- SpatialPointsDataFrame(coords = xxyy, data = midpoints,
                               proj4string = CRS("+proj=sterea +lat_0=46 +lon_0=25 +k=0.99975 +x_0=500000 +y_0=500000 
                              +ellps=krass +towgs84=33.4,-146.6,-76.3,-0.359,-0.053,0.844,-0.84 +units=m +no_defs"))
if (!file.exists("landslide_inventory_midpoints.shp")){
...
shape1@data$step2=as.factor(shape1@data$step2)
summary(shape1@data$step2)
if (!file.exists("landslide_inventory_long_wide_result.shp.shp")){
...
conflevel = 0.95
fourfoldplot(ff1,col=c("blue","red"),conf.level = conflevel)

fourfoldplot(ff2,col=c("blue","red"),conf.level = conflevel)
...
rocobj1 <- plot.roc(shape1@data$step1n, shape1@data$real_typen, percent=TRUE,  ci=TRUE, of="se", 
                   specificities=seq(0, specifity, 5), ci.type="shape", ci.col='#1c61b6AA')
